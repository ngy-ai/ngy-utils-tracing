...
fn main() -> anyhow::Result<()> {
    ...
    let result = init(None, vec!["my_crate".to_string()])?;
...
| -------------------------------- | ------------------------------------------------------------ |
| `init(mode_override, crates)`    | Unified initialization of dotenv + tracing, selecting config by mode. Returns `InitResult` (with `guard` and `mode`). |
